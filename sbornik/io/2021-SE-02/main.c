#include <unistd.h>
#include <fcntl.h>
#include <err.h>
#include <stdint.h>

uint8_t decode(uint16_t a) {
  uint8_t res = 0;

  for(int i = 0, j = 0; i < 16; i+=2, j++) {
    if((a & (1 << i)) == 0) {
      res |= (1 << j);
    }
  }

  return res;
}

int main(int argc, char* argv[]) {
  if(argc != 3) errx(1, "bad args");

  int fd_input = open(argv[1], O_RDONLY);
  if(fd_input < 0) err(1, "Couldn't open");

  int fd_output = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if(fd_output < 0) err(1, "Couldn't open");

  int bytes;
  uint16_t cur;
  while((bytes = read(fd_input, &cur, sizeof(cur))) > 0) {
    uint8_t decoded_data = decode(cur);
    int written = write(fd_output, &decoded_data, sizeof(decoded_data));
    if(written < 0) err(1, "Couldn't read");
    if(written != sizeof(decoded_data)) errx(1, "Couldn't write all");
  }
  if(bytes < 0) err(1, "Couldn't read");

  close(fd_input);
  close(fd_output);
}
